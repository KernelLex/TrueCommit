...
Running list of every claim we make about why this problem matters, each with its source: the ₹8.1 lakh crore MSME receivables figure, ~70% cart abandonment, the two named track directions we cover, the OTM rail gap (Razorpay's WhatsApp integration = one-time payments only; OTM unproductized for delivery-secured e-com). When a claim enters the README or video script, it must exist here first with a source.
