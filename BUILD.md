...
> **Status (2026-08-30): every Day 0–10 item below is built and checked off**, except the two that are date-gated or environment-blocked, not code-blocked — the `v1.0-freeze` git tag (Sep 1, per §3 Day 7) and the 30-second demo gif (§3 Day 9, no screen-recording tool available in this build environment). The project then went substantially further than this original plan, at the user's own explicit direction each time: real phone calls + IVR (Track A), the Auditor (master doc §7.3), Scene 2's full cause→instrument routing, and a 7-packet extension (debit-failure taxonomy, debtor-level judgment, an adversarial red-team suite, mandate-acceptance learning, a real-browser UI pass, and a live multi-channel demo path). **`PROGRESS.md` is the authoritative, current, day-by-day account of all of that; this file is kept as the original plan record and is not rewritten to describe the extension.**
...
- [x] pressing Advance-Day visibly moves money/promises/trust on screen — `dashboard/src/App.jsx`'s "Advance 1 Day ▶" button calls the real `POST /advance`, which drives the actual pipeline and returns `funnel_summary` + a per-day `stories` payload; `FunnelScreen.jsx`/`DayStoryScreen.jsx`/`TrustCurvesScreen.jsx` render it (screens built in packet P10 — this checkbox was stale from before that packet landed). Verified at the data level 2026-08-29 (`POST /advance {"days":3}` moved `recovered_inr` 0 → 7,149 via the two Tier-0 reserve carts, states genuinely changed) and via the dashboard's own dev proxy. **Visually confirmed in a real browser 2026-08-30** (post-freeze-scope UI pass, PROGRESS.md): headless Chromium driven through the actual running app, all six screens screenshotted, zero JS errors, real data rendering correctly throughout.
- [x] Auditor (master doc §7.3): 10% sampling verification + rolling accuracy widget + quarantine flag (<85% → money actions to review queue) — built 2026-08-28, live on the System Health screen. Not part of the original Day 7 scope-cut plan's must-haves at the time this checklist was written, but shipped rather than cut since freeze was still days out; full detail in `PROGRESS.md`.
