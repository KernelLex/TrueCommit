...
- **delivery_dispute** — the debtor reports a delivery/condition problem (damaged, wrong, incomplete goods) but has NOT refused to pay the invoice outright.
- **cashflow_delay** — a genuine behavioral delay; the debtor intends to pay but is cash-constrained right now.
- **dispute** — the debtor contests the invoice itself: refuses payment or disputes the amount/terms. **Scope rule: an outright refusal ("we're not paying this") is `dispute` even when the stated reason is a delivery or quality complaint** — the refusal, not the reason, is what defines it.
- **non_responsive** — the debtor has gone silent despite outreach, with no stated reason at all.
...
3. Invoice 20 days overdue, delivery_confirmed=false, thread: "the upholstery set arrived with 3 damaged panels — please advise how you'll fix this before we process payment" → `{"cause": "delivery_dispute", "confidence": 0.9, "evidence": ["delivery/condition complaint", "payment held pending resolution, NOT refused outright"]}`
4. Invoice 25 days overdue, thread: "we're not paying this, the upholstery set arrived with 3 damaged panels, raised this with your team weeks ago" → `{"cause": "dispute", "confidence": 0.9, "evidence": ["outright payment refusal — scope rule: refusal makes it a dispute even though the stated reason is a delivery complaint"]}`
5. Invoice 25 days overdue, thread: "This fabric lot does not match the sample approved in March. We are disputing this invoice in full." → `{"cause": "dispute", "confidence": 0.9, "evidence": ["debtor disputes the invoice/order itself in full"]}`
6. Invoice 35 days overdue, no replies to two reminders → `{"cause": "non_responsive", "confidence": 0.85, "evidence": ["zero replies across multiple outreach attempts"]}`
7. Invoice 14 days overdue, delivery_confirmed=true (signed POD on file), thread: "not paying, we never received this order" → `{"cause": "dispute", "confidence": 0.6, "evidence": ["debtor claims non-delivery but delivery record contradicts it", "flag for human review given the contradiction"]}`
