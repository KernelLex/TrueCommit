...
### Last updated: 2026-08-28 (Track A / IVR built, live-tested, and a real bug from that live test fixed — see "DONE" below; Track B / WhatsApp remains not started, not currently selected) · Repo: github.com/KernelLex/TrueCommit · Freeze: Sep 1 · Submit: Sep 5
...
### 1. IVR (call → press 1/2 → real mandate or payment link) — ✅ DONE and LIVE-VERIFIED, 2026-08-28
Built per the design already scoped via `AskUserQuestion` before the pause: `Ledger.ivr_select()`/`ivr_available_options()`, `POST /telephony/ivr-menu` + `/telephony/ivr-response` (Twilio-facing webhooks, always return valid TwiML, never an HTTP error), `POST /entities/{id}/call-ivr-now` (operator trigger, gated by `WorldRunner.real_telephony_contact` — same opt-in/credential/real-contact shape as every other real-dispatch path), `telephony.place_ivr_call()`. The real Razorpay call is made directly and unconditionally from the response webhook, bypassing the rate-limited autonomous `_payment_instrument()` path — the precedent packet P13's `create-mandate-now` console established. Multi-language menu and reviving Scene 2 stay explicitly out of scope (the user's own scoping choice, recorded before the pause). New dependency: `python-multipart` (Twilio webhooks POST form-encoded bodies; FastAPI/Starlette's form parser needs it regardless of encoding — found live when the first webhook test failed on it).

**The user placed a genuine live call and it initially failed — found, diagnosed, and fixed the same day.** The call itself worked end to end; the real mandate creation behind pressing 1 did not, because `invoice.due` (always in the past — every invoice in this dataset is deliberately overdue) was being used as a REAL Razorpay `start_at`, which Razorpay's live API correctly rejects. Fixed with `_real_future_debit_date()` (real wall-clock today + 7 days, the exact offset already proven live in the P12 pivot) — the same latent bug existed in `create-mandate-now` too and was fixed there in the same pass. Re-verified against the real Razorpay TEST account, both branches: a real mandate (`sub_TUtUEWeof23qHH`) and a real payment link both created successfully. A second, unrelated bug was found while chasing this one — two pre-existing tests could silently make a real ~20s network call to Google's gTTS endpoint — also found and fixed. 30 new/updated IVR tests, 607/607 total. Full story: `tracking/BUILD_LOG.md`, `tracking/DECISIONS.md`, 2026-08-28.

**Not built, deliberately out of this pass's scope:** a dashboard button to trigger `call-ivr-now` (backend + tests only, matching what was actually asked).
