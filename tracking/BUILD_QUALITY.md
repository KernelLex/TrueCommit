...
## Tests
- Test count passing: 217 / 217 at last full run (169 through batch 1 + 25 `test_ollama_provider.py` [P5, merged] + 23 `test_integration.py` [P2, delivered])
- Last full `pytest tests/ -q` run: 2026-08-26, all green, ~7.5s
...
- Two seeded **integration-runner** runs diffed (`WorldRunner(seed=42).advance(45)` × 2, in two separate processes, dumping audit trail + event stream + message queue + funnel + every entity state): ✅ verified 2026-08-26, byte-identical (213 KB of JSON, `diff` empty). Also asserted in-suite by `test_two_fresh_runners_produce_identical_audit_trails`, plus `test_the_same_days_split_differently_land_on_the_same_world` (12 × `advance(1)` == 1 × `advance(12)` — "Advance 1 Day" and "Run to Day 45" cannot diverge).
